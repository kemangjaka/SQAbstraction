...
class ShapePredModule(nn.Module):
  def __init__(self, params, outChannelsV, biasTerms=None):
    ...
    shapeLayer = nn.Conv3d(outChannelsV, 2, kernel_size=1)
...
'''
class ScalePredModule(nn.Module):
  def __init__(self, params):
    self.scaleModule = nn.Sequential()
'''

class ScalePredModule(nn.Module):
  def __init__(self, params, outChannelsV, biasTerms=None):
    super(ScalePredModule, self).__init__()
    scaleLayer = nn.Conv3d(outChannelsV, 3, kernel_size=1)
    #self.shapeLrDecay = 1
    scaleLayer.apply(nUtils.weightsInit)
    scaleLayer.note = 'scalePred'
    self.gridBound = params.gridBound
    biasTerms = biasTerms
    if biasTerms is not None:
      scaleLayer.bias.data = biasTerms.scale.clone()
    self.scaleLayer = scaleLayer
    self.sigmoid = nn.Sigmoid()

  def forward(self, feature):
    x = self.scaleLayer(feature)
    #x = x * self.shapeLrDecay
    x = self.sigmoid(x) * self.gridBound
    #x = x * self.gridBound
    x = x.view(feature.size(0), -1)
    return x
...
class TransPredModule(nn.Module):
  def __init__(self, params, outChannelsV, biasTerms=None):
    ...
    transLayer = nn.Conv3d(outChannelsV, 3, kernel_size=1)
...
class PrimitivePredModule(nn.Module):
  def __init__(self, params, outChannelsV, biasTerm):
    ...
    self.scalePred = ScalePredModule(params, outChannelsV, biasTerm)
